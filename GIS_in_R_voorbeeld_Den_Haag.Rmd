...
Alleen de p5 vormen zijn eigenlijk niet genoeg voor een kaart. Om een basisreferentie van Den Haag/Nederland te hebben nemen we een OSM als achtergrond. We kunnen bijvoorbeeld ggmap gebruiken.
```{r Den Haag Basemap}
require(ggmap)
...
```
Het lijkt niet gecorreleerd te zijn, maar we testen het lekker toch. Verrassend genoeg is er wel een significante correlatie. p5 gebieden zonder blkoverwarming hebben een gemiddeld energielabel van 4.8 (tussen D en E). Per pand met blokverwarming gaat daar gemiddeld 0.03 af.
```{r test correlatie blokverwarming en energielabel}
fit_lm <- lm(MEAN_label ~ N_blokverwarming, energielabels)
summary(fit_lm)
```
